...
// num arr = [num1, num2]
// a  = b* q +r
// Math.max(numarr) = Math.min(num)*(Math.floor(Math.max(numarr)/Math.min(numarr))) + Math.max(numarr)%Math.mix(numarr)
function rGCF(num1,num2){
    var nums_arr = [num1,num2]
    max = Math.max(...nums_arr)
    min = Math.min(...nums_arr)   
...
console.log(rGCF(123456,9876541))


// Bonus Challenges (Optional)
// Tarai
// The Tarai (Japanese: “to pass around”) function was created to profile recursive performance in various systems and languages. 
// Unlike other functions, numbers don't get particularly large, but the amount of recursion is significant.
...
// The resultant array itself need not be in any specific order – it is the subset of letters 
// in each string that must be in the same order as they were in the original string.
